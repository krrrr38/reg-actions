...
const differences = ({ result, baseUrl, commentReportFormat, }) => {
...
const newItems = ({ result, baseUrl, commentReportFormat, }) => {
...
const deletedItems = ({ result, baseUrl, commentReportFormat, }) => {
...
const createCommentWithTarget = ({ event, runId, regBranch, artifactName, sha: currentHash, targetRun, result, date, customReportPage, disableBranch, commentReportFormat, }) => {
    ...
    const report = (result.failedItems.length === 0 && result.newItems.length === 0 && result.deletedItems.length === 0) ||
        disableBranch
        ? ''
        : `   
<details>
...
const createCommentWithoutTarget = ({ result, artifactName, customReportPage, }) => {
